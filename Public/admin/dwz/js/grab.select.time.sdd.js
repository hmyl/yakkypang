		...
		'<table>'
			...
			,'<tr>'
				...
				,'<td><table>'
					...
					,'</table></td>'
			,'</tr>'
			,'<tr><td>&nbsp;</td><td align="right"><a href="javascript:void(0)">ok</a></td></tr>'
		,'</table>','<table>'
			,'<tr>'
				,'<td>hour</td>'
				,'<td><table>'
	...
	var defOptions = {
 	...
 	function createTable(target){
		...
		$hourTDs.click(function(){
			$(this).parent().parent().find('td').removeClass('time-selected');
 	...
 	function showTimePanel(target){
        ...
        $timePanel.find('td').removeClass('time-selected');
 		$timePanel.show();
 	}
