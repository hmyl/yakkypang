...
class IndeXImagesController extends CommonController {
    public function index(){
        $p = I('post.pageNum',1);
        $row = I('post.numPerPage',20);
        $model = M('index_images');
    ...
    function series(){
        ...
        $series_id = I('get.id');
        $re = M('index_images')->where(['series_id' => $series_id])->select();
    ...
    public function add_series(){
        ...
        $id = I('get.id','0','intval');
        if($id){
            $oldData = D('index_images')->find($id);
            if($oldData['images'])
    ...
    public function addImages(){
        ...
        if($id){
            ...
            $re = M('index_images')->where(['id'=>$id])->save($u);
            $msg = '修改成功';
        }else{
            $u = [
                'photo' => $data['photo'],
            ];
            $re = M('index_images')->add($u);
	
            $msg = '添加成功';
        }
        
        if($re)
            dwz_return($msg,"",'closeCurrent','index_list');
        else
           dwz_error('添加失败'); 
    }
    ...
    public function delimage(){
        ...
        if(!$id||!$src){
            ...
            $image = M('index_images')->where(['id'=>$id])->find();
            ...
            if($image){
                ...
                if($new){
                    ...
                    M('index_images')->where(['id'=>$id])->save(['images'=>$new]);
                }
    ...
    public function delAll(){
        ...
        $series = I('get.ser',0);
        if(1){
            $model = D("index_images");
            M('index_images')->where(['id'=>I('get.ids')])->delete();
            $re['msg'] = '删除成功';
            dwz_return($re['msg'],'','','index_list');
        }else{
            $model = D("IntoHelp");
            $re = $model->del($ids);
            dwz_return($re['msg'],'','','index_list');
